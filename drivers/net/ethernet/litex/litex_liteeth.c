...
#define LITEETH_WRITER_EV_STATUS	0x14
#define LITEETH_WRITER_EV_PENDING	0x18
#define LITEETH_WRITER_EV_ENABLE	0x1c
#define LITEETH_READER_START		0x20
#define LITEETH_READER_READY		0x24
#define LITEETH_READER_SLOT		0x28
#define LITEETH_READER_LENGTH		0x2c
#define LITEETH_READER_EV_STATUS	0x34
#define LITEETH_READER_EV_PENDING	0x38
#define LITEETH_READER_EV_ENABLE	0x3c
#define LITEETH_MAC_PREAMBLE_CRC	0x40
...
static void iowrite32be_len(u32 val, void __iomem *addr, size_t len)
{
	int i;
	u8 out = 0;
	BUG_ON(len > 4);
	...
	for (i = len - 1; i > 0; i--) {
		out = val >> (i * 8);
		iowrite32be(out, addr);
		addr += 4;
	}
	iowrite32be((u8)val, addr);
}

static u32 ioread32be_len(void __iomem *addr, size_t len)
{
	u32 out;
	int i;
...
static int liteeth_rx(struct net_device *netdev)
{
	...
	rx_slot = ioread32be(priv->base + LITEETH_WRITER_SLOT);
	len = ioread32be_len(priv->base + LITEETH_WRITER_LENGTH, 4);
	...
	memcpy(data, priv->rx_base + rx_slot * LITEETH_BUFFER_SIZE, len);
...
static irqreturn_t liteeth_interrupt(int irq, void *dev_id)
{
	...
	reg = ioread32be(priv->base + LITEETH_READER_EV_PENDING);
	if (reg) {
		liteeth_tx_done(netdev);
		iowrite32be(reg, priv->base + LITEETH_READER_EV_PENDING);
	}

	reg = ioread32be(priv->base + LITEETH_WRITER_EV_PENDING);
	if (reg) {
		liteeth_rx(netdev);
		iowrite32be(reg, priv->base + LITEETH_WRITER_EV_PENDING);
	}
...
static int liteeth_open(struct net_device *netdev)
{
	...
	iowrite32be(1, priv->base + LITEETH_WRITER_EV_PENDING);
	iowrite32be(1, priv->base + LITEETH_READER_EV_PENDING);

	/* Enable IRQs? */
	iowrite32be(1, priv->base + LITEETH_WRITER_EV_ENABLE);
	iowrite32be(1, priv->base + LITEETH_READER_EV_ENABLE);
...
static int liteeth_stop(struct net_device *netdev)
{
	...
	iowrite32be(0, priv->base + LITEETH_WRITER_EV_ENABLE);
	iowrite32be(0, priv->base + LITEETH_READER_EV_ENABLE);
...
static int liteeth_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	iowrite32be(priv->tx_slot, priv->base + LITEETH_READER_SLOT);
	iowrite32be_len(skb->len, priv->base + LITEETH_READER_LENGTH, 2);
	...
	iowrite32be(1, priv->base + LITEETH_READER_START);
...
{
	/* Reset, twice */
	iowrite32be(0, priv->base + LITEETH_PHY_CRG_RESET);
	udelay(10);
	iowrite32be(1, priv->base + LITEETH_PHY_CRG_RESET);
	udelay(10);
	iowrite32be(0, priv->base + LITEETH_PHY_CRG_RESET);
	udelay(10);
}
