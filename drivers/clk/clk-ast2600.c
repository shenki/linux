...
static const struct aspeed_gate_data aspeed_g6_gates[] = {
	...
	[ASPEED_CLK_GATE_D1CLK] 	= { 10,  ASPEED_RESET_CRT, 	"d1clk-gate",	"d1clk",	0 }, 		/* GFX CRT */ //reserved 11/12
...
static int aspeed_g6_clk_probe(struct platform_device *pdev)
{
	...
	/* EMMC ext clock divider */
        hw = clk_hw_register_gate(dev, "emmc_extclk_gate", "hpll", 0,
                                  scu_g6_base + ASPEED_G6_CLK_SELECTION, 15, 0,
                                  &aspeed_clk_lock);
        if (IS_ERR(hw))
                return PTR_ERR(hw);
	hw = clk_hw_register_divider_table(dev, "emmc_extclk", "emmc_extclk_gate", 0,
			scu_g6_base + ASPEED_G6_CLK_SELECTION, 12, 3, 0,
			soc_data->div_table,
			&aspeed_clk_lock);
	if (IS_ERR(hw))
		return PTR_ERR(hw);
